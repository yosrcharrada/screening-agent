# api/score_calculator.py - ENHANCED WITH LLM LOGGING
import logging
...
class ScoreCalculator:
    ...
    def calculate_comprehensive_score(self, 
        ...
        # 🚨 ADD EXPLICIT LLM LOGGING HERE
        logger.info("🤖 ========== STARTING LLM EXPLANATION ==========")
        logger.info(f"🤖 LLM Model: {llm_service.fast_model}")
        logger.info(f"🤖 Score: {final_score}/100")
        
        # Add LLM explanation with detailed logging
        try:
            ...
            if llm_explanation and "fallback" not in llm_explanation.lower():
                ...
                result["version"] = "1.1-llm"
                result["llm_model"] = llm_service.fast_model
                result["llm_status"] = "success"
                logger.info("🎉 ✅ LLM EXPLANATION SUCCESSFULLY USED!")
                logger.info(f"💬 LLM RESPONSE: {llm_explanation}")
            else:
                # LLM returned fallback or empty
                result["llm_explanation"] = llm_service._get_fallback_explanation(result)
                result["version"] = "1.0-fallback"
                result["llm_status"] = "fallback_used"
                logger.warning("⚠️ LLM returned fallback explanation")
                
        except Exception as e:
            # LLM completely failed
            result["llm_explanation"] = llm_service._get_fallback_explanation(result)
            result["version"] = "1.0-llm-failed"
            result["llm_status"] = "failed"
            result["llm_error"] = str(e)
            logger.error(f"❌ LLM COMPLETELY FAILED: {e}")
        
        logger.info(f"🤖 ========== LLM STATUS: {result.get('llm_status', 'unknown')} ==========")
